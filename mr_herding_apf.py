...
def run_apf(run_id: int = 0, gui: bool = False):
    entity_config_file = os.path.join(CONFIG_DIR, 'entity_apf.yaml')
    with open(entity_config_file, 'r') as file:
        entity_config = yaml.safe_load(file)
    ...
    # Animal flocking behavior
    math_flock_config = behavior_config['math_flock']['params']
    math_flock = MathematicalFlock(**math_flock_config)
    for animal in animals:
        math_flock.add_animal(animal)
    ...
    PlaygroundFactory.add_dynamic(entities=robots,
                                  ...
                                  dynamic_type=SingleIntegrator)
    # Add behavior as well
    apf_config = behavior_config['herding_apf']['params']
    PlaygroundFactory.add_behavior(entities=robots,
                                   config=apf_config,
                                   behavior_type=DecentralisedAPF,
                                   behavior_name="apf")
    ...
    while env.ok:
        env.step()
        env.render()
    env.save_data(data_name=f"apf_{str(run_id)}",
                  path="data/aggregation_pref/success_rate/low/4/apf/")
